...
- Use `swarmvault graph status [path]` before refreshing a tracked repo when you need to know whether a code-only `graph update` is enough or a full `compile` is required.
