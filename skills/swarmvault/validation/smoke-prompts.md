...
- mentions `wiki/graph/share-card.md`, `wiki/graph/share-card.svg`, `swarmvault graph share --post`, or `swarmvault graph share --svg` when the user wants a copyable or visual summary
...
- may suggest `swarmvault graph share --post` when a quick copyable summary is enough, or `swarmvault graph share --svg [path]` for a visual card
