...
各 compile は、ビジュアル版と投稿向けの share card も生成します：
...
- **Share card** —— `wiki/graph/share-card.md`、`wiki/graph/share-card.svg`、`swarmvault graph share --post`、`swarmvault graph share --svg` によるコピー可能かつ視覚的な初回サマリー
...
ローカル repo や docs ツリーを最短で一度見たい場合は、`swarmvault scan ./path --no-serve` を使います。現在のディレクトリを vault として初期化し、そのディレクトリを取り込み、compile まで実行し、`--no-serve` ならグラフビューアは起動しません。`wiki/graph/share-card.md` と `wiki/graph/share-card.svg` も残るため、`swarmvault graph share --post` で短い共有用サマリーを出力し、`swarmvault graph share --svg ./share-card.svg` でビジュアルカードを生成できます。
...
**ビジュアル + 投稿しやすい share card** - すべての compile が `wiki/graph/share-card.md` と `wiki/graph/share-card.svg` を生成します。`swarmvault graph share --post` は短いテキストを出力し、`swarmvault graph share --svg [path]` はソース/ページ/ノード数、top hubs、最も意外なリンクを含む 1200x630 のビジュアルカードを書き出します。
