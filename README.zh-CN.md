...
当生成的 vault artifacts 需要离开项目根目录时，可以设置 `SWARMVAULT_OUT=.swarmvault-out`，例如 scratch worktree 或 package smoke test。`swarmvault.config.json` 与 `swarmvault.schema.md` 仍保留在项目根目录；相对的 `raw/`、`wiki/`、`state/`、`agent/` 和 `inbox/` 路径会解析到输出目录下。
